...
local function autoLockLoop()
    while autoLockEnabled do
        ...
        pcall(function()
            ...
            local plot = workspace.Plots and workspace.Plots[plotId]
            if not plot or not plot:FindFirstChild("Lock") then return end

            local lock = plot.Lock
            if lock:GetAttribute("LockState") == "Idle" then
                local root = player.Character:FindFirstChild("HumanoidRootPart")
                if root then
                    -- Try both common interaction methods
                    if firetouchinterest then
                        firetouchinterest(root, lock.Pad, 0)
                        task.wait(0.1)
                        firetouchinterest(root, lock.Pad, 1)
                    elseif fireproximityprompt then
                        fireproximityprompt(lock.Pad)
                    end
                end
            end
        end)
...
local function startAutoLock()
    ...
    autoLockThread = coroutine.create(autoLockLoop)
    coroutine.resume(autoLockThread)
end

local function stopAutoLock()
    if autoLockThread then
        coroutine.close(autoLockThread)
        autoLockThread = nil
    end
end
